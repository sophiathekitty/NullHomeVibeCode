 ...
 *   - A live list of Wemo devices found during the scan.
 *   - A Cancel button (halts the loop mid-scan).
 *   - A Close button (appears when the scan is finished or cancelled).
 */

/**
 * Initialise the Wemo scan overlay.
 *
 * Binds the "Scan for Wemos" drawer button to open the overlay and wires up
 * the Start, Cancel, and Close buttons inside the overlay.
 *
 * @returns {void}
 */
function initWemoScan() {
    var overlay      = document.getElementById('wemoScanOverlay');
    var statusEl     = document.getElementById('wemoScanStatus');
    var progressWrap = document.getElementById('wemoScanProgressWrap');
    var progressFill = document.getElementById('wemoScanProgressFill');
    var progressLbl  = document.getElementById('wemoScanProgressLabel');
    var foundList    = document.getElementById('wemoScanFoundList');
    var btnStart     = document.getElementById('wemoScanStart');
    var btnCancel    = document.getElementById('wemoScanCancel');
    var btnClose     = document.getElementById('wemoScanClose');
    var openBtn      = document.getElementById('openWemoScan');
    ...
    /**
     * Append a found Wemo entry to the results list.
     *
     * @param {string} ip   - The IP address of the found device.
     * @param {string} name - The friendly name of the device.
     * @returns {void}
     */
    function addFoundWemo(ip, name) {
        foundList.hidden = false;
        var li = document.createElement('li');
        li.className = 'wemo-scan-found-item';
        var nameSpan = document.createElement('span');
        nameSpan.className = 'wemo-scan-found-name';
        nameSpan.textContent = name;
        var ipSpan = document.createElement('span');
        ipSpan.className = 'wemo-scan-found-ip';
    ...
    async function runNextStep() {
        ...
        try {
            ...
            if (data.result === 'found_wemo' || data.result === 'known_wemo') {
                addFoundWemo(data.ip, data.name || data.ip);
            }
